...
class MockDataGenerator {
  ...
  static createMockPaymentRequest(data: any): PaymentRequest {
    ...
    return {
      ...
      status: 'pending',
      paymentLink: `https://pay.SplitPay.com/p/${id.substr(-8)}`,
      qrCodeUrl: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://pay.SplitPay.com/p/${id.substr(-8)}`,
      ...
      participants: data.participants?.map((p: any, index: number) => ({
        ...
        amount: p.amount,
        hasPaid: false,
        participantLink: `https://pay.SplitPay.com/split/${id.substr(-8)}_${index}`
      }))
    };
  }
}
