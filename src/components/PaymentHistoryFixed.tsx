...
export function PaymentHistory({ onNavigate }: PaymentHistoryProps) {
  ...
  const PaymentCard = ({ payment, isActive = false }: { payment: PaymentRequest; isActive?: boolean }) => (
    <Card className={`p-4 ${isActive ? 'border-l-4 border-l-blue-500' : ''}`}>
      ...
      {isActive && payment.type === 'group_split' && payment.participants && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {activePayments.length > 0 ? (
              activePayments.map((payment) => (
                <PaymentCard key={payment.id} payment={payment} isActive />
              ))
            ) : (
